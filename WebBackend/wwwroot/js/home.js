$(function () {
  ajaxPie(1, "echarts-pie-chart1", '游览内容', '最近三十天点击量');
  ajaxPie(2, "echarts-pie-chart2", '关注内容', '最近三十天咨询量');
  ajaxLine(1, "echarts-line-chart1", '七日访问量', '点击量');
  ajaxLine(2, "echarts-line-chart2", '七日留言数', '留言数');
});

function ajaxLine(viewType, domid, text, subtext) {
  $.ajax({
...
function ajaxPie(viewType, domid, text, subtext) {
  $.ajax({
    url: "/Home/GetViewLogReports",
    contentType: "application/json",
    type: "POST", // 请求类型，通常是 GET 或 POST
    data: JSON.stringify({
      viewType: viewType
